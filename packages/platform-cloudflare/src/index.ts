...
const CLOUDLFARE_TO_CORE_STATUS: Record<
...
export const createCloudflareWorkflowRuntime = ({
  ...
  const get = async (
    ...
    const recoveredBase: CommerceWorkflowRunRecord = {
      ...
      input: undefined,
      runId,
      status: CLOUDLFARE_TO_CORE_STATUS[status.status],
      updatedAt: now,
      workflowKey: "unknown",
      workflowVersion: 0,
    };
  ...
  return {
    ...
    dedupe: async <Output = unknown>({
      ...
      const runId = idempotencyIndex.get(`${workflowKey}:${idempotencyKey}`);
      return runId
        ? (((await get(runId)) as CommerceWorkflowRunRecord<unknown, Output>) ??
            null)
        : null;
    },
    ...
    reconcile: async <Output = unknown>(
      ...
      const reconciled: CommerceWorkflowRunRecord = {
        ...
        status: CLOUDLFARE_TO_CORE_STATUS[instanceStatus.status],
        updatedAt: clock.now(),
      };
    ...
    start: async <Input, Output>(
      ...
      const duplicateKey = request.idempotencyKey
        ? `${request.workflow.key}:${request.idempotencyKey}`
        : undefined;

      if (duplicateKey) {
        const duplicateRunId = idempotencyIndex.get(duplicateKey);
        if (duplicateRunId) {
          const duplicate = await get(duplicateRunId);
          if (duplicate) {
      ...
      runs.set(runId, run);
      if (duplicateKey) {
        idempotencyIndex.set(duplicateKey, runId);
      }

      await persistMetadata(metadataStore, run);
      await publishLifecycleEvent({
        ids,
        metadataStore,
