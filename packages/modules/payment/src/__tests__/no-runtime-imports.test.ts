...
const forbiddenImport = (specifier: string): boolean =>
  ...
  specifier.toLowerCase().includes("paykit") ||
  specifier.toLowerCase().includes("stripe");

const forbiddenRuntimeTokenPattern =
  /\b(?:DurableObjectNamespace|DurableObjectStub|DurableObjectState|PayKit|Stripe)\b/;
...
describe("payment module boundaries", () => {
  it("does not import legacy API, schema, database, runtime-specific modules, or concrete payment SDKs", () => {
