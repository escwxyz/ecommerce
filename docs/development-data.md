...
checkout prerequisites for inventory, tax, and fulfillment data. It does not
create store records, customer records, product records, product variants,
region records, sales-channel records, pricing records, auth users, carts,
orders, payments, fulfillments, or notification history.

The store tracer slice and customer/product/region-sales-channel/pricing
...
and pricing calculation while checkout and its dependent modules remain on the
legacy runtime. The server composition therefore provides temporary
deterministic compatibility facades for that path. Those facades are owned by
the checkout migration gap and must be deleted in task 8.6, when checkout
orchestration moves to Effect and consumes migrated module service Layers
directly.
...
`pricing_money_amount`, `pricing_price_rule`, or `pricing_price_preference`
