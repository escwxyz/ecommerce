...
- [ ] 2.3 Define Effect service and Layer conventions for modules, adapters, request scope, transactions, and tests
...
- [ ] 3.1 Define runtime-neutral repository, transaction, migration, and outbox service contracts
...
- [ ] 3.6 Implement transactional outbox persistence and claiming primitives
...
- [ ] 5.1 Define Effect Schema identity, session, role, permission, and auth error contracts
- [ ] 5.2 Define the Effect-native `AuthService` and request-auth context services
- [ ] 5.3 Wrap existing Better Auth behavior in a private Effect adapter Layer
- [ ] 5.4 Translate Better Auth requests, results, and failures without leaking Better Auth types
- [ ] 5.5 Integrate the auth adapter with Effect HTTP middleware and protected API groups
...
- [ ] 6.7 Replace the store oRPC router with Effect admin/storefront API groups and handlers
...
- [ ] 7.5 Migrate inventory end-to-end and delete its legacy path
...
- [ ] 8.9 Run cross-module commerce golden-path, failure, permission, and concurrency suites

## 9. Durable workflows, outbox, queues, and actors

- [ ] 9.1 Define schema-versioned workflow definitions, state, step outcomes, retry, and compensation contracts
- [ ] 9.2 Implement the deterministic in-memory workflow runtime and recovery test harness
...
- [ ] 11.1 Select and implement the first Cloudflare telemetry exporter Layer
