...
- **BREAKING** Replace Kysely and its migration/query abstractions with Effect SQL, raw SQL, module-owned repository contracts, and adapter-owned migrations and implementations.
...
- `effect-sql-persistence`: Defines raw Effect SQL persistence, module repository contracts, D1-first adapter ownership, migrations, transactions, outbox semantics, and future dialect conformance.
...
- `commerce-data-adapters`: Replace Kysely database adapters with Effect SQL adapter packages and raw SQL implementations.
...
- Requires a new D1 schema baseline; no production backfill or legacy API compatibility is required.
- Preserves Cloudflare D1, Workers, Queues, Durable Objects, Worker Loader, Alchemy, the Medusa-inspired module model, and metadata-driven admin extensibility as deployment and product goals rather than core runtime dependencies.
