...
Cloudflare Durable Objects SHALL be the first actor adapter, and D1 SHALL remain authoritative relational storage unless a separate accepted design assigns ownership to actor-local storage.

#### Scenario: Durable Object stores local state
- **WHEN** an actor persists coordination, cache, or workflow state
- **THEN** its ownership and recovery relationship to D1 MUST be explicit and tested
